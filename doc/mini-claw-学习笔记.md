...
────────────────────────────────────────────
Telegram  ─┐
WhatsApp  ─┤→  Gateway  →  Orchestrator  →  Agent A
HTTP API  ─┘   （谁进来）    （谁来干）        Agent B
```

**Gateway** 关心的问题：
- 消息从哪个平台来？（Telegram / HTTP / Discord）
- 这个用户有没有权限？
- 路由到哪个 session？（chat_id → Agent 实例）

**Orchestrator（编排器）** 关心的问题：
- 这个任务该交给哪个 Agent 执行？（意图判断）
- 并行还是串行？
- 多个 Agent 的结果怎么合并？

mini-claw 目前只有 Gateway，没有独立的 Orchestrator——所有消息进来都交给同一个 Agent。OpenClaw 的设计是没有单独的 Orchestrator 层，而是让 Agent 本身兼任 Orchestrator，通过工具调用 spawn 子 Agent，形成树状结构。`subagent-depth.ts` 就是限制这棵树的深度，防止无限嵌套。

---

### 两层老管家 — LLM 和代码守门层的分工

在 agentic 系统里，"谁来做决定"这个问题有一个关键的分层：

**第一层老管家（LLM）**：聪明、灵活，但概率性。它决定调用哪个工具、什么顺序、什么参数。你给它最好的 prompt，它大概率遵守，但不是 100%。

**第二层老管家（代码守门层）**：笨、不灵活，但确定性。它不理解意图，不接受解释，只做硬检查——路径对不对、命令在不在白名单、域名有没有被允许。
...
**为什么不能用 prompt 替代代码边界？**

prompt 是给 LLM 看的请求，LLM 是概率性的。在 prompt 里写"不许访问 workspace 外的文件"，是在请求 LLM 自律。`_is_safe_path()` 写在代码里，LLM 根本没有机会"决定"要不要遵守——调用到那里就是直接拒绝。

> **prompt 里的规则** → LLM 可以"理解错"、被 inject、偶尔忽略
> **代码里的规则** → 物理边界，LLM 不参与这个决定

两者可以共存，但代码边界不能被 prompt 替代。这也是为什么 Step 2（安全设计）在 Step 3 之前——在给 Agent 任何能力之前，先把框焊死。
...
守门人不是系统里的一个独立模块，而是横切所有执行点的防线——每个"能产生真实世界效果"的点，都有一道独立的硬检查：

```
工具调用 read_file   → 检查路径是否在 workspace 内
工具调用 bash        → 检查命令是否在白名单
工具调用 fetch_url   → 检查域名是否被允许
工具调用 write_file  → 检查路径 + 检查写权限
```

不是一道总闸，是每个窟窿都单独堵上。**能力开放到哪里，守门人就跟到哪里。**

**2. Hooks 是最后的拦截窗口**

Claude Code 的 `PreToolUse` hook 之所以重要，不是因为可以格式化代码，而是因为它是"LLM 做了决定"和"工具真正执行"之间唯一还能插入确定性逻辑的地方：

```
LLM 决定调用 Bash "rm -rf ..."
        ↓
PreToolUse Hook  ← 最后的拦截窗口
        ↓
真正执行（不可逆）
```

这个窗口一旦过了就没了。所以 hooks 不是锦上添花的小工具，是安全架构的一部分。

**3. 紧箍咒 — 力量和控制权是两回事**

唐僧打不过任何一个妖怪，但他掌握紧箍咒，孙悟空就得跪。`_is_safe_path()` 不比 LLM 聪明，但 LLM 绕不过它。

力量和控制权，不是同一件事。越聪明的守门人，反而越危险——因为聪明意味着有判断空间，有判断空间就有被说服、被误导、被 inject 的可能。一个会"理解上下文"的守门人，比一个只做字符串比较的守门人更危险。

**4. 能力和约束必须同步扩张**

猪八戒、沙和尚不需要紧箍咒，因为他们能力有限，能捅的娄子也有限。孙悟空大闹天宫，约束成本才值得。

给 Agent 加一个新工具，第一件事不是想这个工具能做什么，而是想**这个工具可能被滥用来做什么**，然后在工具层堵上那个窟窿。能力和约束，永远不能让能力跑在约束前面。

---

### Skill 厚度和模型能力成反比

Skill 不是独立存在的东西，它是"模型能力 + 工具覆盖 + Agent 自由度"三者的函数。

**模型越弱，Skill 越厚**：需要手把手告诉它每个步骤，列出 1234，规定输出格式，约束推理路径。但这样做的副作用是 Agent 变得死板——你跟它聊句天，它也会试图列出执行计划。

**模型越强，Skill 越薄**：只给目标和约束，它自己判断是否需要规划、要不要分步骤。Skill 不再是流程描述，而变成"能力边界声明"。

三个前提缺一个，Skill 都会失效：
- 模型不够强 → Skill 再薄也跑不起来
- 工具不存在 → Skill 输出的 JSON 没人接得住
- Agent 自由度被锁死 → Skill 再聪明，Agent 也只能按固定路径走

**实习生 vs Senior 类比**：给实习生要一整本 SOP，给 Senior 一页纸就够了。写 Skill 之前先问：我用的是哪种模型？

---

### 安全来自结构，不来自理解

当系统足够复杂，人已经无法理解每一行代码时，靠审查来保安全是幻觉。

**靠理解**：找所有漏洞，逐个堵上。问题是：你永远不确定找完了没有，模型越强越可能留你看不出来的"后门"。

**靠结构**：画一个圈，不需要知道圈里发生了什么，只需要确保出不了圈。Docker 隔离、workspace 边界、只读文件系统、无网络权限——哪怕里面真有后门代码，它也没有资源和路径执行真正的破坏。

```
监狱的安全不来自于"知道犯人每天在干什么"
而来自于"他翻不过墙"
```

mini-claw 的 `_is_safe_path()` 就是这个思路的最小实现——LLM 有没有"理解"边界不重要，调用到那里就是结构上拒绝。

**推论**：模型越强，"画圈"比"查漏补洞"越重要。强模型有更强的搜索能力，能找到你没想到的路径绕过规则——但它跑不出沙箱。这也是为什么模型越强，Orchestrator 的边界设计反而越关键。
